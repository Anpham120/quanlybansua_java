...
Đây là cơ hội quý báu để chúng em vận dụng tổng hợp kiến thức đã học trong môn Lập trình Java: lập trình hướng đối tượng, Java Swing, JDBC, MySQL và các mẫu thiết kế như DAO–MVC, đồng thời rèn luyện kỹ năng làm việc nhóm, phân công công việc và quản lý mã nguồn trong một dự án thực tế.
...
- Chương 1: Công nghệ, Thư viện và Mẫu thiết kế
...
| Bảng 3 | Các mẫu thiết kế áp dụng |
| Bảng 4 | Danh sách Ca sử dụng |
...
| Số hình | Tên hình |
|---|---|
| Hình 1 | Sơ đồ Ca sử dụng tổng quan |
| Hình 2a | Sơ đồ lớp – Tầng Mô hình (6 POJO) |
| Hình 2b | Sơ đồ lớp – Tầng DAO (7 lớp) |
| Hình 2c | Sơ đồ lớp – Tầng GUI + Utils (16 lớp) |
...
|---|---|---|---|---|
| 1 | Phạm Duy An | BIT240002 | Kiến trúc tổng thể; `Main` (**CompletableFuture** khởi tạo CSDL bất đồng bộ); `DatabaseConnection` (tự động khởi tạo CSDL + dữ liệu mẫu); `LoginFrame`; `MainFrame` (phân quyền CardLayout); `DoiMatKhauDialog`; `UIConstants` (hệ thống giao diện); `AppConstants`; **`EventBus`** (kiến trúc Phát/Đăng ký xử lý thông điệp) | **20%** |
| 2 | Bùi Đào Đức Anh | BIT240025 | `SanPhamPanel` (**đăng ký lắng nghe EventBus** tự làm mới sau bán hàng); `SanPhamDAO`; Model `SanPham`; Kiểm tra ngày NSX/HSD; Tô màu hàng sắp hết / hết hàng; Lọc sản phẩm theo loại; `InputFilter`; `TableHelper` | **20%** |
| 3 | Đỗ Tuấn Anh | BIT240015 | `KhachHangPanel`; `KhachHangDAO`; Model `KhachHang` (getHang, getTyLeGiam); `NhanVienPanel`; `NhanVienDAO`; Model `NhanVien`; Kiểm tra tuổi nhân viên (≥ 18) | **20%** |
| 4 | Nguyễn Quang Hiếu | BIT240082 | `BanHangPanel` (**Stream API + Optional** xử lý giỏ hàng; **phát sự kiện EventBus** sau thanh toán); `TaiKhoanPanel`; `TaiKhoanDAO`; Model `TaiKhoan`; Logic chiết khấu; Cộng điểm tích lũy; Đăng ký khách hàng nhanh tại quầy | **20%** |
| 5 | Phan Văn Hiếu | BIT240094 | `HoaDonPanel`; `HoaDonDAO` (JDBC Transaction + thống kê khoảng ngày); `ThongKePanel` (**SwingWorker** tải dữ liệu bất đồng bộ; **đăng ký lắng nghe EventBus** tự làm mới); Biểu đồ cột (`Graphics2D`); Xuất CSV; Model `HoaDon`; Model `ChiTietHoaDon` | **20%** |
...
|---|---|---|
| Đối tượng truy cập dữ liệu (DAO) | Tầng dao/ | Tách biệt SQL khỏi GUI |
| Model–View–Controller (MVC) | Toàn bộ kiến trúc | Dễ bảo trì, mở rộng |
| Nhà máy tĩnh (biến thể Singleton) | DatabaseConnection | Quản lý kết nối tập trung |
| Phương thức khuôn mẫu (biến thể) | UIConstants | Giao diện nhất quán |
| Quan sát (Observer) | Swing EventListener | Phản ứng sự kiện người dùng |
| Phát/Đăng ký (EventBus) | `utils/EventBus` | Giao tiếp lỏng lẻ giữa các module |

### 1.2.1. Đối tượng truy cập dữ liệu (DAO)
...
> **[Chèn Hình 2a: Sơ đồ lớp – Tầng Mô hình – xuất hình từ `diagrams/ClassDiagram_Model.puml`]**

> **[Chèn Hình 2b: Sơ đồ lớp – Tầng DAO – xuất hình từ `diagrams/ClassDiagram_DAO.puml`]**

> **[Chèn Hình 2c: Sơ đồ lớp – Tầng GUI + Utils (16 lớp, bao gồm EventBus) – xuất hình từ `diagrams/ClassDiagram_GUI.puml`]**
...
Java Swing tích hợp sẵn mẫu Quan sát thông qua hệ thống sự kiện: các lớp Panel đóng vai tròbên quan sát bằng cách đăng ký `ActionListener`, `DocumentListener`, `ListSelectionListener`... lên các thành phần. Khi người dùng tương tác, Swing tự động kích hoạt hàm gọi lại tương ứng.

### 1.2.6. Phát/Đăng ký tùy chỉnh (EventBus)

Ngoài hệ thống sự kiện tích hợp sẵn của Swing, nhóm tự triển khai lớp `EventBus` trong `utils/EventBus.java` theo mô hình **Phát — Đăng ký** để cho phép các module giao tiếp lỏng lẻ (không phụ thuộc trực tiếp):

- **Bên phát:** `BanHangPanel` phát sự kiện `THANH_TOAN` và `CAP_NHAT_SAN_PHAM` sau khi hoàn tất giao dịch.
- **Bên lắng nghe:** `ThongKePanel` và `SanPhamPanel` đăng ký lắng nghe các sự kiện trên và tự động cập nhật dữ liệu mà không cần tham chiếu trực tiếp lẫn nhau.

Thiết kế sử dụng `Map<String, List<Consumer<Object>>>` — kết hợp **Kiểu tổng quát** (Generic), **Tập hợp** (Collections) và **Giao diện hàm** (`Consumer<T>`).

```java
// Bên phát (BanHangPanel — sau khi thanh toán thành công):
EventBus.publish(EventBus.SU_KIEN_THANH_TOAN, idHoaDon);
EventBus.publish(EventBus.SU_KIEN_CAP_NHAT_SAN_PHAM, null);

// Bên lắng nghe (ThongKePanel — trong hàm khởi tạo):
EventBus.subscribe(EventBus.SU_KIEN_THANH_TOAN, data -> taiDuLieu());

// Bên lắng nghe (SanPhamPanel — trong hàm khởi tạo):
EventBus.subscribe(EventBus.SU_KIEN_CAP_NHAT_SAN_PHAM, data -> taiDuLieu());
```

Lợi ích: khi thêm module mới (ví dụ: module Nhập hàng), chỉ cần đăng ký lắng nghe sự kiện tương ứng mà không phải sửa bất kỳ module cũ nào.

## 1.3. Lập trình không đồng bộ

Ứng dụng desktop cần đảm bảo giao diện luôn phản hồi ngay cả khi thực hiện các thao tác tốn thời gian như truy vấn cơ sở dữ liệu. Java Swing chạy trên một luồng duy nhất gọi là **Luồng phân phối sự kiện** (EDT) — nếu truy vấn CSDL trên luồng này, giao diện sẽ bị đóng băng.

Dự án áp dụng 2 kỹ thuật bất đồng bộ:

### 1.3.1. SwingWorker — Tải dữ liệu nền trong ThongKePanel

`ThongKePanel` cần gọi 6 truy vấn CSDL mỗi lần cập nhật (doanh thu, số HĐ, cảnh báo, biểu đồ, Top 5). Nhóm sử dụng `SwingWorker<Map, Void>` để:

1. **`doInBackground()`**: Chạy trên luồng nền — gọi 6 truy vấn CSDL mà không ảnh hưởng giao diện.
2. **`done()`**: Chạy trên luồng giao diện — nhận kết quả và cập nhật các thẻ KPI, biểu đồ, bảng Top 5.
...
new SwingWorker<Map<String, Object>, Void>() {
    ...
    protected void done() {
        ...
        capNhatKPI((double) r.get("doanhThu"), ...);
    }
}.execute();
```

### 1.3.2. CompletableFuture — Khởi tạo CSDL bất đồng bộ

Trong `Main.java`, việc tạo cơ sở dữ liệu, bảng và dữ liệu mẫu được thực hiện bất đồng bộ bằng `CompletableFuture.runAsync()`:
...
CompletableFuture.runAsync(() -> DatabaseConnection.khoiTaoDatabase())
    ...
    .exceptionally(ex -> {
        // Xử lý lỗi kết nối CSDL — hiện hộp thoại cảnh báo
        return null;
    });
```

- `runAsync()`: Chạy tác vụ trên nhóm luồng mặc định (ForkJoinPool).
- `thenRun()`: Chuỗi hành động tiếp theo — hiển thị màn hình đăng nhập sau khi CSDL sẵn sàng.
- `exceptionally()`: Xử lý ngoại lệ — hiện hộp thoại lỗi khi không kết nối được MySQL.

## 1.4. Lập trình hàm

Java 8+ cung cấp các tính năng lập trình hàm giúp viết mã ngắn gọn và biểu đạt ý đồ rõ ràng hơn. Dự án sử dụng lập trình hàm ở các vị trí sau:

### 1.4.1. Luồng dữ liệu (Stream API)

Luồng dữ liệu được sử dụng thay vì vòng lặp truyền thống để xử lý tập hợp dữ liệu:

```java
// Tính tổng tiền giỏ hàng (BanHangPanel):
double tongGoc = gioHang.stream()
    .mapToDouble(ChiTietHoaDon::getThanhTien)
    .sum();

// Kiểm tra tồn kho đã đặt (BanHangPanel):
int daCoTrongGio = gioHang.stream()
    .filter(ct -> ct.getIdSanPham() == idSP)
    .mapToInt(ChiTietHoaDon::getSoLuong)
    .sum();

// Luồng số nguyên thay thế vòng lặp for (BanHangPanel):
IntStream.range(0, gioHang.size()).forEach(i -> {
    ChiTietHoaDon ct = gioHang.get(i);
    modelBangGio.addRow(new Object[]{...});
});

// Tìm giá trị lớn nhất trên biểu đồ (ThongKePanel):
double maxVal = data.stream()
    .mapToDouble(r -> (double) r[1])
    .max().orElse(1);
```

### 1.4.2. Optional — Tránh lỗi tham chiếu null

`Optional<T>` được sử dụng khi tìm kiếm có thể không trả về kết quả:

```java
// Tìm sản phẩm trùng trong giỏ hàng (BanHangPanel):
...
### 1.4.3. Giao diện hàm — Consumer, Predicate

Lớp `EventBus` sử dụng `Consumer<Object>` — giao diện hàm chuẩn của Java — làm kiểu dữ liệu cho bên lắng nghe:

```java
// Consumer<Object> là giao diện hàm
EventBus.subscribe("THANH_TOAN", data -> capNhatThongKe());

// Phát sự kiện sử dụng Optional + forEach (lập trình hàm)
Optional.ofNullable(subscribers.get(event))
    .ifPresent(handlers -> handlers.forEach(h -> h.accept(data)));
```

### 1.4.4. Biểu thức Lambda và Tham chiếu phương thức

Biểu thức Lambda được sử dụng xuyên suốt dự án cho xử lý sự kiện Swing và giao diện hàm:
...
gioHang.stream().mapToDouble(ChiTietHoaDon::getThanhTien);
```
...
## 2.1. Danh sách Ca sử dụng

*(Bảng 4 – Danh sách Ca sử dụng)*

| Mã UC | Tên Ca sử dụng | Tác nhân | Mô tả |
|---|---|---|---|
...
> **Lưu ý:** UC01 (Đăng nhập) là điều kiện tiên quyết bắt buộc cho tất cả Ca sử dụng khác.

> **[Chèn Hình 1: Sơ đồ Ca sử dụng – xuất hình từ file `diagrams/UseCase.puml`]**
...
## 2.3. Luồng xử lý Bán hàng (Ca sử dụng chính)

UC09 – Bán hàng & Thanh toán là Ca sử dụng trung tâm của hệ thống. Nhân viên tìm kiếm sản phẩm theo tên (lọc theo thời gian thực), chọn số lượng và thêm vào giỏ hàng. Hệ thống cho phép chọn khách hàng thành viên từ danh sách hoặc nhập SĐT để tra cứu — nếu tìm thấy sẽ tự động áp tỷ lệ chiết khấu tương ứng; nếu không tìm thấy sẽ đề xuất đăng ký mới. Nếu không chọn khách hàng, đơn hàng được ghi là "Khách lẻ" (không chiết khấu, không tích điểm). Khi nhân viên xác nhận thanh toán, `HoaDonDAO.taoHoaDon()` thực hiện toàn bộ thao tác trong một JDBC Transaction: tạo hóa đơn, ghi chi tiết từng sản phẩm, trừ tồn kho và cộng điểm tích lũy. Nếu bất kỳ bước nào thất bại, toàn bộ giao dịch được ROLLBACK.
...
2. **Thêm:** Nhập tên, loại, thương hiệu, đơn giá, số lượng, NSX, HSD → hệ thống kiểm tra:
...
3. **Sửa:** Click hàng → form điền dữ liệu → chỉnh sửa → nhấn **Sửa** → kiểm tra tương tự (bỏ qua trùng tên chính nó) → gọi `SanPhamDAO.capNhat()` → `UPDATE` → refresh.
...
2. **Thêm:** Nhập họ tên, SĐT, địa chỉ → kiểm tra:
...
3. **Sửa:** Click hàng → chỉnh sửa thông tin (Quản lý có thể sửa điểm tích lũy, Nhân viên không) → kiểm tra trùng SĐT (bỏ qua chính KH đang sửa) → gọi `KhachHangDAO.capNhat()`.
...
2. **Thêm:** Nhập họ tên, SĐT, địa chỉ, ngày sinh, giới tính, chọn tài khoản → kiểm tra:
...
3. **Sửa:** Click hàng → chỉnh sửa → kiểm tra tương tự → `NhanVienDAO.capNhat()`.
...
2. **Thêm:** Nhập tên đăng nhập, mật khẩu, chọn vai trò → kiểm tra:
...
3. **Sửa:** Click hàng → form điền dữ liệu → chỉnh sửa → kiểm tra tương tự (bỏ qua trùng TK đang sửa) → `TaiKhoanDAO.capNhat()`.
...
2. **Lọc:** Nhập SĐT khách hàng và/hoặc khoảng ngày (Từ ngày – Đến ngày, định dạng `dd/MM/yyyy`) → nhấn **Lọc** → hệ thống kiểm tra:
...
1. Mở tab **Thống kê** → hệ thống tự động tải dữ liệu (tự làm mới mỗi lần chuyển sang tab):
...
2. **Lọc khoảng ngày:** Chọn Từ ngày – Đến ngày (JSpinner Date) → nhấn **Lọc theo khoảng** → cập nhật KPI, biểu đồ (nếu khoảng ≤ 31 ngày) và bảng Top 5 theo khoảng ngày tùy chọn. Kiểm tra: "Từ ngày" phải trước "Đến ngày".
...
Tất cả nút bấm sử dụng `UIConstants.createStyledButton()` — phương thức tạo thiết lập `setBackground()`, `setOpaque(true)`, `setBorderPainted(false)` để đảm bảo màu hiển thị đúng dưới Nimbus L&F.
...
| ngay_sinh | DATE | — | Ngày sinh (kiểm tra ≥ 18 tuổi) |
...
- Kiểm tra: tên không trống + không trùng, thương hiệu bắt buộc, đơn giá > 0, số lượng ≥ 0, HSD > NSX.
...
- Kiểm tra: họ tên ≥ 2 ký tự chỉ chấp nhận chữ cái, SĐT 10 số bắt đầu bằng 0 + không trùng.
...
- Kiểm tra: họ tên ≥ 2 ký tự chỉ chứa chữ cái, tuổi ≥ 18, SĐT 10 số bắt đầu bằng 0 + không trùng, địa chỉ ≥ 10 ký tự.
- ComboBox "Tài khoản" chỉ hiện TK vai trò NV chưa liên kết.
- Tìm kiếm theo tên hoặc SĐT.

## 5.6. Module Hóa đơn

- Lọc theo khoảng ngày (kiểm tra định dạng `dd/MM/yyyy`, "Từ ngày" ≤ "Đến ngày"), SĐT khách hàng.
...
- **Bộ lọc khoảng ngày:** JSpinner Date → "Lọc theo khoảng" → cập nhật KPI + biểu đồ (nếu ≤ 31 ngày) + Top 5. Kiểm tra "Từ ngày" trước "Đến ngày".
...
- Kiểm tra: tên đăng nhập ≥ 4 ký tự, không khoảng trắng, không trùng (`kiemTraTrungTenDN`); mật khẩu ≥ 6 ký tự.
...
- Kiểm tra: mật khẩu mới ≥ 4 ký tự, khớp xác nhận, khác MK cũ, đúng MK cũ.
...
- **Tự động hóa triển khai:** Tự khởi tạo CSDL + bảng + 17 sản phẩm mẫu.
...
- **Lập trình không đồng bộ:** `SwingWorker` trong `ThongKePanel` đảm bảo giao diện không bị đóng băng khi tải dữ liệu; `CompletableFuture` khởi tạo CSDL bất đồng bộ trong `Main`.
- **Lập trình hàm (FP):** Stream API, Optional, Lambda và Method Reference được sử dụng thay thế vòng lặp truyền thống trong xử lý giỏ hàng và thống kê.
- **Kiến trúc thông điệp:** `EventBus` (Pub/Sub) cho phép các module giao tiếp lỏng lẻ — `BanHangPanel` tự động đồng bộ `ThongKePanel` và `SanPhamPanel` sau thanh toán.

## Hạn chế

- Mật khẩu lưu **plain-text** — chưa hash bằng BCrypt/SHA-256.
- Chưa có chức năng **in hóa đơn** ra PDF.
- Giao diện **chưa tương thích đa kích thước** — kích thước cố định.
...
[10] Oracle Corporation, "CompletableFuture (Java SE 17)," *Oracle Docs*, 2023. [Online]. Available: https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/util/concurrent/CompletableFuture.html. [Accessed: Mar. 2026].

[11] Oracle Corporation, "SwingWorker (Java SE 17)," *Oracle Docs*, 2023. [Online]. Available: https://docs.oracle.com/en/java/javase/17/docs/api/java.desktop/javax/swing/SwingWorker.html. [Accessed: Mar. 2026].

[12] Baeldung, "Guide to Java Streams," *Baeldung*, 2024. [Online]. Available: https://www.baeldung.com/java-streams. [Accessed: Mar. 2026].
...
│   │   ├── DatabaseConnection.java  ← Tự khởi tạo CSDL + dữ liệu mẫu
...
│   │   ├── BanHangPanel.java        ← Stream/Optional/LTH + phát sự kiện EventBus
│   │   ├── SanPhamPanel.java        ← lắng nghe EventBus
│   │   ├── KhachHangPanel.java
│   │   ├── NhanVienPanel.java
│   │   ├── HoaDonPanel.java
│   │   ├── ThongKePanel.java        ← SwingWorker + lắng nghe EventBus
│   │   ├── TaiKhoanPanel.java
...
│       ├── EventBus.java            ← Phát/Đăng ký thông điệp giữa module
│       ├── InputFilter.java         ← DocumentFilter (chỉ số)
│       └── TableHelper.java         ← Kẻ sọc, nhãn, nút tùy chỉnh
│
├── lib/
...
└── diagrams/
    ...
    ├── ClassDiagram_GUI.puml      ← 10 lớp GUI + 6 lớp Utils + Entry
