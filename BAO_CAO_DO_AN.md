...
3. **Sửa:** Click hàng → form điền dữ liệu → chỉnh sửa → nhấn **Sửa** → validate tương tự → gọi `SanPhamDAO.sua()` → `UPDATE` → refresh.
4. **Xóa:** Chọn hàng → nhấn **Xóa** → hộp thoại xác nhận → gọi `SanPhamDAO.xoa()` → `DELETE` → refresh.
5. **Tìm kiếm:** Gõ tên vào ô tìm kiếm → `DocumentListener` lọc realtime → gọi `SanPhamDAO.timTheoTen()`.
6. **Lọc theo loại:** Chọn loại từ `ComboBox` → gọi `SanPhamDAO.layTheoLoai()`.
...
3. **Sửa:** Click hàng → chỉnh sửa thông tin (không sửa được điểm thủ công) → validate → gọi `KhachHangDAO.sua()`.
4. **Xóa:** Chọn hàng → xác nhận → nếu KH đã có hóa đơn → hỏi xác nhận → chuyển hóa đơn thành "Khách lẻ" (`id_khach_hang = NULL`) → xóa KH.
5. **Tìm kiếm:** Gõ tên hoặc SĐT → lọc tức thì.
...
3. **Sửa:** Click hàng → chỉnh sửa → validate tương tự → `NhanVienDAO.sua()`.
4. **Xóa:** Chọn hàng → xác nhận → nếu NV đã lập hóa đơn → **không cho xóa** (hiện cảnh báo).
5. **ComboBox Tài khoản:** Chỉ hiển thị các tài khoản vai trò "Nhân viên" chưa được liên kết bởi nhân viên khác + tài khoản hiện tại đang chọn.

## 2.10. Luồng CRUD Tài khoản

**Tác nhân:** Quản lý

1. Mở tab **Tài khoản** → bảng hiển thị tên đăng nhập, vai trò.
2. **Thêm:** Nhập tên đăng nhập, mật khẩu, chọn vai trò → validate:
   - Tên đăng nhập không trống
   - Mật khẩu ≥ 4 ký tự
   - Tên đăng nhập không trùng (`UNIQUE` constraint)
   → Gọi `TaiKhoanDAO.them()` → `INSERT`.
3. **Sửa:** Click hàng → chỉnh sửa → validate → `TaiKhoanDAO.sua()`.
4. **Xóa:** Chọn hàng → xác nhận → gọi `TaiKhoanDAO.xoaCoLienKet()`:
   - Kiểm tra TK có liên kết nhân viên không → nếu có: SET `id_tai_khoan = NULL` trong `nhan_vien`
   - Kiểm tra NV liên kết có hóa đơn không → nếu có: SET `id_nhan_vien = NULL` trong `hoa_don`
   - Xóa tài khoản
...
1. Mở tab **Hóa đơn** → bảng trên hiển thị danh sách hóa đơn (mã, tên NV, tên KH, ngày, tổng tiền).
2. **Lọc:** Chọn khoảng ngày (Từ ngày – Đến ngày) và/hoặc nhập SĐT khách hàng → nhấn **Lọc** → gọi `HoaDonDAO.layDanhSach()` với bộ lọc tương ứng.
3. **Xem chi tiết:** Click vào một hóa đơn → bảng dưới hiển thị danh sách sản phẩm trong hóa đơn đó (tên SP, số lượng, đơn giá, thành tiền) từ bảng `chi_tiet_hoa_don`.

## 2.12. Luồng Thống kê

**Tác nhân:** Quản lý

1. Mở tab **Thống kê** → hệ thống tự động tải dữ liệu:
   - **5 thẻ KPI:** Doanh thu hôm nay, doanh thu tháng, số hóa đơn hôm nay, SP sắp hết hạn (trong 7 ngày), SP hết hàng (tồn kho = 0).
   - **Biểu đồ cột:** Doanh thu 7 ngày gần nhất, vẽ bằng `Graphics2D`.
   - **Top 5 SP bán chạy:** Sắp xếp theo tổng số lượng bán, tô màu vàng/bạc/đồng cho 3 vị trí đầu.
2. **Lọc khoảng ngày:** Chọn Từ ngày – Đến ngày → nhấn **Lọc** → cập nhật biểu đồ và bảng Top 5 theo khoảng ngày tùy chọn.
3. **Xuất CSV:** Nhấn **Xuất CSV** → chọn nơi lưu file → hệ thống ghi dữ liệu doanh thu + Top 5 ra file `.csv` với UTF-8 BOM, tương thích mở trực tiếp bằng Excel.
